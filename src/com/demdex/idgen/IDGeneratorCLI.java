...
//		logger = LoggerFactory.getLogger(IDGeneratorCLI.class);
	...
	public void read(String category) throws Exception {
		...
		System.out.println("Read idSet: "+idSet.takeID() + "====" + idSet.takeID());
	}
	...
	private void ensureParentCreated(File category) throws KeeperException, InterruptedException {
		...
		Stat parentStat = zookeeper.exists(parent.getPath(), false/*=watch*/);
		if (parentStat != null) {
			return;
		}
		doCreate(parent, "");
...
//	private static final List<ACL> DEFAULT_ACL = ZooDefs.Ids.CREATOR_ALL_ACL;
